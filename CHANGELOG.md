...
- 凭证返回 HTTP `401` 或 `429` 时自动禁用，避免持续使用失效或限流凭证。
- 账号池卡片新增凭证启用/禁用手动开关。
- 禁用状态写入账号配置并在重启后保留。

### Changed

- 已禁用凭证不会参与负载均衡、优先级调度和自动刷新。
- 手动重新启用凭证后会立即重新检测账号状态。
...
- 概览页账号状态改为紧凑表格，大量账号时可在独立区域滚动查看。
- 集中显示账号类型、状态、Go 最高用量、Zen 状态和最近检查时间。
- Web 侧栏默认版本号同步为 `0.7.2`。

### Fixed

- 修复 Windows 双击运行时因 `8787` 端口已占用而直接闪退的问题。
- 已有网关运行时自动打开现有管理页面；其他程序占用端口时保留错误窗口。
