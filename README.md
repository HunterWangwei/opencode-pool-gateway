...
涓嬭浇 `opencode-pool-gateway-0.7.1-windows-amd64.exe`锛屽弻鍑昏繍琛岋紝鐒跺悗璁块棶锛?
```text
http://localhost:8787
```
...
```bash
chmod +x opencode-pool-gateway-0.7.1-linux-amd64
./opencode-pool-gateway-0.7.1-linux-amd64
```

ARM64 Ubuntu 浣跨敤 `opencode-pool-gateway-0.7.1-linux-arm64`銆傛湇鍔″櫒閮ㄧ讲鍙?systemd 閰嶇疆瑙?[docs/ubuntu.md](docs/ubuntu.md)銆?
## 鐧诲綍瀹夊叏
...
```bash
./opencode-pool-gateway-0.7.1-linux-amd64 --version
```
